...
void UpdateOTACanvasDraw(uint8_t mode, M5EPD_Canvas *update_canvas) {
...
void key_cupdate_mode_cb(epdgui_args_vector_t &args) {
    ...
    UpdateOTACanvasDraw(mode, canvas);
...
void key_cupdate_reset_cb(epdgui_args_vector_t &args) {
    M5.EPD.FillPartGram4bpp(104, 168, 432, 748, 0xFFFF);
    M5.EPD.UpdateArea(104, 168, 432, 748, UPDATE_MODE_INIT);
}

Frame_OTA::Frame_OTA(void) {
    _frame_name = "Frame_OTA";
    ...
    exitbtn("Home");
        _canvas_title->drawString("OTA", 270, 34);
...
int Frame_OTA::init(epdgui_args_vector_t &args) {
    _is_run      = 1;
    _update_flag = 1;
    M5.EPD.Clear();
