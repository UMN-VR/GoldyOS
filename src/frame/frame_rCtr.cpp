...
void UpdaterCtrCanvasDraw(uint8_t mode, M5EPD_Canvas *update_canvas) {
...
void key_rupdate_mode_cb(epdgui_args_vector_t &args) {
    ...
    UpdaterCtrCanvasDraw(mode, canvas);
...
void key_rupdate_reset_cb(epdgui_args_vector_t &args) {
    M5.EPD.FillPartGram4bpp(104, 168, 432, 748, 0xFFFF);
    M5.EPD.UpdateArea(104, 168, 432, 748, UPDATE_MODE_INIT);
}

Frame_rCtr::Frame_rCtr(void) {
    _frame_name = "Frame_rCtr";
    ...
    exitbtn("Home");
    _canvas_title->drawString("rCtr", 270, 34);
...
int Frame_rCtr::run() {
    ...
    switch (_update_flag) {
        case 1: {
            _update_flag = 0;
            for (int i = 1; i < 8; i++) {
                UpdaterCtrCanvasDraw(i, _canvas);
...
int Frame_rCtr::init(epdgui_args_vector_t &args) {
    _is_run      = 1;
    _update_flag = 1;
    M5.EPD.Clear();
